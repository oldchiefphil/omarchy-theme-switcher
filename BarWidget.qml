...
BarWidget {
  ...
  function showHeadline(message) {
    if (headlineLoader.item && headlineLoader.item.show) {
      // Pin the banner to this bar's output before showing it. The stock
      // picker maps without an explicit screen (the compositor picks the
      // output under the cursor — where the bar was just clicked), so
      // anchoring to our own button's screen keeps the banner on the same
      // monitor as the picker. If no window context exists (e.g. a CLI
      // launch), screenSurface stays null and the compositor picks — the
      // same rule the picker itself runs under.
      var win = null
      var anchor = root.button
      if (anchor) {
        var qw = anchor.QsWindow
        win = qw ? qw.window : null
      }
      headlineLoader.item.screenSurface = win ? win.screen : null
      headlineLoader.item.show(message)
    }
  }
