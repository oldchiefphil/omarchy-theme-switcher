...
Item {
  ...
  property bool debug: boolSetting("debug", false)
  ...
  readonly property var effectiveCoords: {
    // The source matching the toggle drives the schedule; the toggle's
    // lossless alternatives only fall back to the IP detect. Stored manual
    // coordinates must never hijack auto mode (and the stored weather.json
    // only serves manual mode as a last resort): a city picked earlier with
    // auto off must not silently move the sun plan while auto is on.
    if (useAutoLocation) {
      if (hasAutoCoords)
        return { latitude: autoLocation.latitude, longitude: autoLocation.longitude }
      if (hasIpCoords)
        return { latitude: ipLocation.latitude, longitude: ipLocation.longitude }
      return null
    }
    if (hasManualCoords)
  ...
  function refresh() {
    if (Object.keys(root.settings).length === 0) return
    // An external theme pick must not silently end a manual override: the
    // reconcile still sees the override window and drops the pick instead.
    // Only a "real" refresh (boot, settings changes) steps the override.
    if (!root._externalThemeSet)
  ...
  // Watch callback: consume only the freshly completed lines (a path split
  // across chunks would otherwise run twice). Every weather.json event
  // reloads the FileView and re-probes the authoritative file contents, so
  // deletes and recreations with identical content are caught too.
  property int _fsPos: 0
  // Set when an external theme change current/theme.name arrives while no
  // own switch is in flight: the reconcile then adopts the pick as the
  // active slot instead of treating it as drift on the auto-schedule.
  property bool _externalThemeSet: false

  function onFsTick() {
    var whole = locWatchCollector.text
    if (whole.length < root._fsPos) {
      root._fsPos = 0
      return
    }
    var idx = whole.lastIndexOf("\n")
    if (idx < root._fsPos) return
    var part = whole.substring(root._fsPos, idx)
    root._fsPos = idx + 1
    var lines = part.split("\n")
    for (var i = 0; i < lines.length; i++) root.onFsLine(lines[i])
  }

  function startLocationWatch() {
    locWatchProc.running = true
  }

  // Recursive inotify only pre-adds subdirectories that exist at startup.
  // A settings/ dir created later (fresh IP-detected install) therefore
  // re-arms the watcher and probes immediately, closing the create race.
  function onFsLine(line) {
    var path = String(line || "").trim()
    if (path === "") return
    if (path.indexOf("/settings") >= 0 && path.indexOf("/settings/") < 0) {
      locWatchProc.running = false
      locationWatchRestartTimer.start()
      locationProbe.running = true
      return
    }
    if (path.indexOf("current/theme.name") >= 0) {
      // External set through the stock menu/picker (`omarchy theme set`):
      // adopted as the active phase's slot. Own switches are the writer
      // too, so only react when none is in flight (the sameTheme check in
      // adoptExternalPick is the second line of defense).
      if (root.enabled && !switchProc.running && !root._switchQueued) {
        root._externalThemeSet = true
        refreshDebounce.restart()
      }
      return
    }
    if (path.indexOf("settings/weather.json") < 0) return
    locationFile.reload()
    locationProbe.running = true
  }
  ...
  property string _runningSwitchTarget: ""
  ...
  function doSwitch(target, reason, fromTheme) {
    ...
    if (switchProc.running || root._switchQueued) {
      // A reconcile re-run that arrives while a switch for the same theme is
      // still in flight would re-apply the theme right after it landed and
      // rotate the staged 0-chosen wallpaper to the theme default. Drop it.
      var activeTarget = switchProc.running
        ? root._runningSwitchTarget
        : (root._switchQueued ? root._switchQueued.target : "")
      if (activeTarget === String(target)) {
        root.dbg("doSwitch dropping duplicate for", target)
        return
      }
      if (!root._switchQueued)
        root._switchQueued = { target: String(target), reason: String(reason || "Switch"), fromTheme: String(fromTheme || "") }
      return
    }
    root._switchQueued = null
    root._runningSwitchTarget = String(target)
  ...
  function onCurrentTheme(raw) {
    ...
    if (purpose === "reconcile") {
      var adoptPick = root._externalThemeSet
      root._externalThemeSet = false
      if (root.overridden) return
      if (Object.keys(root.settings).length === 0) return
      if (adoptPick) {
        root.adoptExternalPick()
        return
      }
  ...
  // A theme picked through Omarchy's STOCK menu (`omarchy theme set`) is
  // adopted into the ACTIVE slot: saved as the scheduled theme for the
  // current phase so the next reconcile is a no-op instead of a fight.
  // The pick always wins for the phase it happened in.
  function adoptExternalPick() {
    var current = String(root.currentTheme || "").replace(/^\s+|\s+$/g, "")
    if (current === "") return
    var slot = root.desiredNow === "dark" ? "darkTheme" : "lightTheme"
    if (Model.sameTheme(current, String(root.settings[slot] || ""))) {
      root.lastAction = current + " active (picked via menu)"
      return
    }
    var entry = { id: String(root.moduleName || "") }
    for (var existing in root.settings) {
      if (existing !== "id") entry[existing] = root.settings[existing]
    }
    // Paranoia: the legacy minute offsets must not re-enter the config.
    ...
    if (root.bar && root.bar.shell && typeof root.bar.shell.updateEntryInline === "function")
      root.bar.shell.updateEntryInline(String(root.moduleName || ""), entry)
    var phase = slot === "darkTheme" ? "dark" : "light"
    root.lastAction = "Adopted " + current + " as " + phase + " theme"
    root.dbg("adoptExternalPick slot=", slot, "theme=", current, "via stock menu pick")
  }
  ...
  onSettingsChanged: {
    refreshDebounce.restart()
    // Settings arrive asynchronously after boot; an early resolveIpLocation
    // may have bailed before the toggle/mode landed. Re-arm it here (it is
    // throttled to one attempt per hour, so settings churn stays cheap) so
    // sun + auto mode gets its IP detect even when weather.json is empty.
    root.resolveIpLocation()
  }
  onActiveChanged: {
  ...
  Component.onCompleted: {
    bgMapFile.reload()
    refresh()
    root.startLocationWatch()
    resumeWatchProc.running = true
    clockWatchProc.running = true
  ...
  // A suspend freezes the process: QML timers are monotonic, so afterwards
  // they keep counting the pre-suspend interval and a dark/light moment can
  // pass on the wall clock without the event timer ever firing. React to
  // the real events instead of polling: wake-up (login1.PrepareForSleep)
  // and clock correction (timedate1.PropertiesChanged, i.e. the NTP step
  // that brings a drifted RTC back). Both feed the same cheap re-plan that
  // the hourly heartbeat already uses; nothing runs in between.
  function onTimeOrResume() {
    root.resolveIpLocation()
    if (root.overridden) root.refreshQuiet()
    else refreshDebounce.restart()
  }

  property int _resumePos: 0
  property bool _pendingSleep: false

  function onResumeTick() {
    var whole = resumeWatchCollector.text
    if (whole.length < root._resumePos) {
      root._resumePos = 0
      return
    }
    var idx = whole.lastIndexOf("\n")
    if (idx < root._resumePos) return
    var part = whole.substring(root._resumePos, idx)
    root._resumePos = idx + 1
    var lines = part.split("\n")
    for (var i = 0; i < lines.length; i++) root.onResumeLine(lines[i])
  }

  function onResumeLine(line) {
    var l = String(line || "").trim()
    if (l === "") return
    // login1.PrepareForSleep: the member line is followed by a boolean;
    // `false` means the machine just resumed.
    if (l.indexOf("member=PrepareForSleep") >= 0) {
      root._pendingSleep = true
      return
    }
    if (root._pendingSleep && l.indexOf("boolean") >= 0) {
      root._pendingSleep = false
      if (l.indexOf("false") >= 0) root.onTimeOrResume()
    }
  }

  property int _clockPos: 0

  function onClockTick() {
    var whole = clockWatchCollector.text
    if (whole.length < root._clockPos) {
      root._clockPos = 0
      return
    }
    var idx = whole.lastIndexOf("\n")
    if (idx < root._clockPos) return
    var part = whole.substring(root._clockPos, idx)
    root._clockPos = idx + 1
    var lines = part.split("\n")
    for (var i = 0; i < lines.length; i++) {
      // timedate1.PropertiesChanged: the clock got adjusted (NTP step after
      // a drifted RTC, timezone change, ...) — re-plan with the corrected time.
      if (String(lines[i] || "").indexOf("path=/org/freedesktop/timedate1") >= 0)
        root.onTimeOrResume()
    }
  }
  ...
  // Kernel-backed alternative to polling: one resident inotifywait streams
  // the state directory's filesystem events, and any weather.json
  // create/write/delete line re-probes the file immediately. No periodic
  // timer and no process churn; the only activity is the change itself.
  // The parent directory is watched recursively so a freshly created
  // settings/ dir (IP auto-detect installs) is seen too.
  Process {
    id: locWatchProc
    command: ["inotifywait", "-m", "-r", "--quiet",
      "--event", "create", "--event", "close_write",
      "--event", "moved_to", "--event", "moved_from", "--event", "delete",
      "--format", "%w%f",
      Quickshell.env("HOME") + "/.local/state/omarchy"]
    stdout: StdioCollector {
      id: locWatchCollector
      waitForEnd: false
      onDataChanged: root.onFsTick()
    }
    onExited: locationWatchRestartTimer.start()
  }

  // Never a periodic poll: re-arms the watcher only if it ever exits.
  Timer {
    id: locationWatchRestartTimer
    interval: 1000
    repeat: false
    onTriggered: locWatchProc.running = true
  }

  // Suspend/resume + clock-correction watcher. dbus-monitor blocks with
  // zero CPU until a matching signal arrives, mirroring the inotifywait
  // watcher above: the process only speaks when the system resumes or the
  // clock is stepped — exactly the moments the monotonic eventTimer misses.
  // Two direct children (no shell wrapper): QProcess reaps them on shell
  // restarts, so no orphans accumulate.
  Process {
    id: resumeWatchProc
    command: ["dbus-monitor", "--system",
      "type='signal',interface='org.freedesktop.login1.Manager',member='PrepareForSleep'"]
    stdout: StdioCollector {
      id: resumeWatchCollector
      waitForEnd: false
      onDataChanged: root.onResumeTick()
    }
    onExited: resumeWatchRestartTimer.start()
  }

  Timer {
    id: resumeWatchRestartTimer
    interval: 1000
    repeat: false
    onTriggered: resumeWatchProc.running = true
  }

  Process {
    id: clockWatchProc
    command: ["dbus-monitor", "--system",
      "type='signal',interface='org.freedesktop.timedate1.Manager'"]
    stdout: StdioCollector {
      id: clockWatchCollector
      waitForEnd: false
      onDataChanged: root.onClockTick()
    }
    onExited: clockWatchRestartTimer.start()
  }

  Timer {
    id: clockWatchRestartTimer
    interval: 1000
    repeat: false
    onTriggered: clockWatchProc.running = true
  }
  ...
  SystemClock {
    ...
    // pill/panel labels fresh. The resume/clock watchers share this path,
    // so nothing polls in between.
    onDateChanged: root.onTimeOrResume()
  }
