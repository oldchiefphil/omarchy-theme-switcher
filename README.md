...
- Sunrise/sunset are computed locally with the NOAA solar equations
  (`SunCalc.js`, no network). The plugin only reaches the network on
  explicit user action: the city-name search below (and the IP
  auto-detect with no stored coordinates).
- When auto-location is off, the manual location section asks for
  **latitude/longitude** directly — that works fully offline. The city-name
  field is an optional *online* search (open-meteo geocoding, same provider
  as the weather panel): press Enter with a name and pick a result to fill
  the coordinates. Without an internet connection it says so in the panel
  instead of silently doing nothing; the coordinates fields below still work.
